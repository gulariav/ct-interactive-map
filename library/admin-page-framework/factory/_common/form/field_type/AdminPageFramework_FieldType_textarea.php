...
class CtInteractiveMap_AdminPageFramework_FieldType_textarea extends CtInteractiveMap_AdminPageFramework_FieldType {
...
jQuery( document ).ready( function(){
    ...
    var updateFoundEditors = function( oElements ) {
        ...
        oElements.each( function( iIndex ) {
            ...
            var oSettings = jQuery().getCtInteractiveMap_AdminPageFrameworkInputOptions( oWrap.attr( 'data-id' ) );   
    ...
    jQuery().registerCtInteractiveMap_AdminPageFrameworkCallbacks( {	
        ...
        repeated_field: function( oCloned, aModel ) {
            ...
            var _oSettings = jQuery().getCtInteractiveMap_AdminPageFrameworkInputOptions( _oWrap.attr( 'data-id' ) );  

            // Elements
            var _oField              = oCloned.closest( '.ct-interactive-map-field' );
        ...
        stopped_sorting_fields : function( oSortedFields, sFieldType, sFieldsTagID, iCallType ) { 
            ...
            // Update the editor.
            setTimeout(function(){
                var _oFields = oSortedFields.children( '.ct-interactive-map-field' );
        ...
        stopped_sorting_sections: function( oSections ) {
            ...
            setTimeout(function(){
                var _oFields = jQuery( oSections ).find( '.ct-interactive-map-field' );
                updateFoundEditors( _oFields );           
            }, 100 );
        ...
        saved_widget : function( oWidget ) { 
            ...
            jQuery( oWidget ).find( '.ct-interactive-map-field' ).each( function( iIndex ) {
                ...
                // Store the settings.
                jQuery().storeCtInteractiveMap_AdminPageFrameworkInputOptions( 
                    oWrap.attr( 'data-id' ), 
                    { 
...
JAVASCRIPTS;
    ...
    protected function getStyles() {
        return ".ct-interactive-map-field-textarea .ct-interactive-map-input-label-string {vertical-align: top;margin-top: 2px;} .ct-interactive-map-field-textarea .wp-core-ui.wp-editor-wrap {margin-bottom: 0.5em;}.ct-interactive-map-field-textarea.ct-interactive-map-field .ct-interactive-map-input-label-container {vertical-align: top; float: left;clear: both;} .postbox .ct-interactive-map-field-textarea .ct-interactive-map-input-label-container {width: 100%;}.ct-interactive-map-field-textarea textarea {max-width: 100%;}";
    }
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField) {
        ...
        $_sClassSelector = $_bIsArray ? 'ct-interactive-map-field-textarea-multiple-labels' : '';
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $sKey, $aField['label']), "<div class='ct-interactive-map-input-label-container {$_sClassSelector}'>", "<label for='" . $aField['attributes']['id'] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField['label']), $_sLabel ? "<span " . $this->getLabelContainerAttributes($aField, 'ct-interactive-map-input-label-string') . ">" . $_sLabel . "</span>" : '', $this->_getEditor($aField), $this->getElementByLabel($aField['after_input'], $sKey, $aField['label']), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $aField['label']),);
        return implode('', $_aOutput);
    }
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='ct-interactive-map-textarea-enabler'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    }
