...
class CtInteractiveMap_AdminPageFramework_Form_View___Script_RepeatableField extends CtInteractiveMap_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updateCtInteractiveMap_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.ct-interactive-map-repeatable-field-buttons' )
            .filter( function() {                                       
                return $( this ).closest( '.ct-interactive-map-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            });       
...
// @todo event.preventDefault();        
            ...
            $( this ).addCtInteractiveMap_AdminPageFrameworkRepeatableField();
...
// @todo event.preventDefault();        
            $( this ).removeCtInteractiveMap_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.ct-interactive-map-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.ct-interactive-map-field' )
            .filter( function() {                 
                return $( this ).closest( '.ct-interactive-map-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            })        
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addCtInteractiveMap_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addCtInteractiveMap_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.ct-interactive-map-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.ct-interactive-map-fields' );
        ...
        if ( ! _aOptions ) {     
            var _nodeButtonContainer = nodeFieldContainer.find( '.ct-interactive-map-repeatable-field-buttons' )
                .filter( function() {                 
                    return $( this ).closest( '.ct-interactive-map-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.ct-interactive-map-field' ) 
                                    .filter( function() {                 
                                        return $( this ).closest( '.ct-interactive-map-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.ct-interactive-map-repeatable-field-buttons' )
                                    .filter( function() {                 
                                        return $( this ).closest( '.ct-interactive-map-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    }); 
         ...
         */
        nodeNewField.updateCtInteractiveMap_AdminPageFrameworkRepeatableFields();
        ...
        $( nodeNewField ).find( '.ct-interactive-map-field' ).addBack().trigger( 
            'ct-interactive-map_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.ct-interactive-map-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {                      
                return $( this ).closest( '.ct-interactive-map-fields' ).attr( 'id' ) === _sFieldsContainerID;                
            })
        ...
        nodeFieldsContainer.children( '.ct-interactive-map-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.ct-interactive-map-field,.ct-interactive-map-fields,.ct-interactive-map-fieldset' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeCtInteractiveMap_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.ct-interactive-map-field' );
        var nodeFieldsContainer = $( this ).closest( '.ct-interactive-map-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
