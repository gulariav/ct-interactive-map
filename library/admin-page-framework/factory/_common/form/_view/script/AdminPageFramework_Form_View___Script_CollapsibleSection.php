...
class CtInteractiveMap_AdminPageFramework_Form_View___Script_CollapsibleSection extends CtInteractiveMap_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializeCtInteractiveMap_AdminPageFrameworkCollapsibleSections();
    });              
     ...
     * @since    3.7.0
     */
    jQuery( document ).bind( 'ct-interactive-map_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializeCtInteractiveMap_AdminPageFrameworkCollapsibleSections();
    });    
    
    $.fn.initializeCtInteractiveMap_AdminPageFrameworkCollapsibleSections = function() {
        
        // Expand collapsible sections that are set not to collapse by default 
        jQuery( this ).find( '.ct-interactive-map-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.ct-interactive-map-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.ct-interactive-map-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.ct-interactive-map-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.ct-interactive-map-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.ct-interactive-map-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.ct-interactive-map-collapsible-sections-title, .ct-interactive-map-collapsible-section-title' )
            .enableCtInteractiveMap_AdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.ct-interactive-map-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'ct-interactive-map-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.ct-interactive-map-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.ct-interactive-map-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.ct-interactive-map-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.click( function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.ct-interactive-map-sectionset' ).siblings().andSelf().find( '> .ct-interactive-map-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.ct-interactive-map-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .ct-interactive-map-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.ct-interactive-map-sections' ).children( '.ct-interactive-map-section' ).children( '.ct-interactive-map-section-table' ).children( 'caption' ).children( '.ct-interactive-map-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
     ...
     */
    $.fn.enableCtInteractiveMap_AdminPageFrameworkCollapsibleButton = function() {
        ...
        function _isFieldElement( nodeTarget ) {
            ...
            if ( jQuery( nodeTarget ).hasClass( 'ct-interactive-map-collapsible-button' ) ) {
                return false;
            }
        ...
        jQuery( this ).click( function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'ct-interactive-map-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.ct-interactive-map-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .ct-interactive-map-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.ct-interactive-map-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='ct-interactive-map-collapsible-toggle-all-button-container'>" . "<span class='ct-interactive-map-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
    }
